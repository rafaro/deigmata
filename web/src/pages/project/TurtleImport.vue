    ...
    <div class="shadow-2 rounded-borders q-pa-md">
      ...
      <template v-else>
        <div class="row items-start q-col-gutter-md q-mb-md">
          <div class="col">
            <div class="text-h5">{{ t('turtleKg.pageTitle') }}</div>
          ...
          <div class="col-auto">
            <div class="row q-gutter-sm">
              <q-btn color="positive" icon="visibility" :label="t('turtleKg.openKg')" @click="goToKg" />
              <q-btn
                flat
                color="grey-7"
                icon="arrow_back"
        ...
        <q-banner rounded class="bg-blue-1 text-primary q-mb-md">
          {{ t('turtleKg.pageBanner') }}
        </q-banner>

        <TurtleKgImportPanel v-model="graph" :project-id="project.id" />

        <q-banner v-if="graphStats" rounded class="bg-green-1 text-positive q-mt-md">
          {{ t('turtleKg.graphSaved', graphStats) }}
        </q-banner>
      </template>
    </div>
  </q-page>
...
<script setup>
  ...
  import TurtleKgImportPanel from 'components/TurtleKgImportPanel.vue'
  ...
  const loadProject = async () => {
    ...
    try {
      ...
      graph.value = response.data.kg || null
    } catch (error) {
